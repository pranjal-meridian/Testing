...
# Load the reference face embedding (to compare against during task)
...
existing_face_embedding = get_face_embedding(r"C:\Users\pranj\OneDrive\Desktop\meridian\Project\testing_images\reference\reference1.jpg")
...
# Task performance detection using head pose estimation
...
for _ in range(3):  # Run for 3 tasks
    ...
    if last_frame is not None:
        # Perform liveness and face matching checks only after the task time ends
        name, similarity = compare_faces(last_frame, existing_face_embedding)
        liveness_status = check_spoofing(last_frame)

        # Display match and liveness status after task time
        print(f"Face Match: {name} (Similarity: {similarity:.2f})")
